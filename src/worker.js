const global = {
  ...
  ctxSpFuncs: ["createLinearGradient", "strokeText", "fillText", "quadraticCurveTo", "bezierCurveTo", "arcTo", "drawImage", "putImageData", "getImageData"],
  map: new Map(),
  lowObjs: [],
  objs: [],
  advancedObjs: [],
...
function run() {
  ...
  let image_bitmap = global.canvas.transferToImageBitmap();
  self.postMessage({ mark: "transferBuffer", data: { buffer: image_bitmap } }, [image_bitmap]);
...
function obj({ x, y, w, h, r, draw, zIndex, uid }) {
  ...
  let obj = new Obj({ x, y, w, h, r, draw, zIndex, uid });
  global.map.set(uid, obj);
  ...
  initCtx() {
    ...
    for (let i in global.co) {
      ...
      Object.defineProperty(this.ctx, i, {
        get() {
          return Object.prototype.toString.call(global.co[i]) === "[object Function]" ? global.co[i].bind(global.co) : global.co[i];
        },
        set(e) {
          global.co[i] = e;
        },
...
self.addEventListener("message", (msg) => {
  ...
  switch (msg.mark) {
    case "size":
      global.canvas = new OffscreenCanvas(data.width, data.height);
    ...
    case "events":
      if (global.top) {
        console.log(1)
        emit(global.top, data.name, { x: data.x, y: data.y });
      }
      break;
    case "registEvent":
      global.map.get(data.uid).on(data.name, data.cb);
      break;
    case "run":
      run();
  }
